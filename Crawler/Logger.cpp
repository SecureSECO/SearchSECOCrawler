...
{
    loguru::log(loguru::Verbosity_INFO, file, line, "%s", getMessage(message, code));
}
void Logger::logWarn(const char* message, const char* file, unsigned int line, int code)
{
    loguru::log(loguru::Verbosity_WARNING, file, line, "%s", getMessage(message, code));
}
void Logger::logFatal(const char* message, const char* file, unsigned int line, int code)
{
    loguru::log(loguru::Verbosity_ERROR, file, line, "%s", getMessage(message, code));
    errno = 1;
}
void Logger::logDebug(const char* message, const char* file, unsigned int line, int code)
{
    loguru::log(1, file, line, "%s", getMessage(message, code));
}
